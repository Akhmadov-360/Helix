...
import { myOrgListResponseSchema, orgMemberListResponseSchema } from "@helix/api-schemas";
...
// FR-ORG-2: орги ТЕКУЩЕГО пользователя (не одной оргы) — под org-switcher в сайдбаре.
export function myOrgsQueryOptions() {
  return queryOptions({
    queryKey: queryKeys.myOrgs(),
    queryFn: () => request({ path: "/v1/organizations/mine", schema: myOrgListResponseSchema }),
    staleTime: 120_000,
  });
}
