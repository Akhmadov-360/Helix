...
**`PROJECT_STORAGE_QUOTA_BYTES = 250 * 1024 * 1024` (250 MB) — суммарная квота на проект.**
Мотивация не UX, а стоимость: место в S3/MinIO не бесплатно, без предела пользователи затаскивают
произвольный объём. Фиксированная константа для этого прохода (нет ни тарифного плана, ни поля
биллинга в домене — вводить их сейчас было бы спекулятивной структурой); когда появится
тарификация аккаунта владельца, эта цифра станет производной от плана организации. Проверяется
дважды, тем же паттерном double-check, что размер файла: (1) на upload-url — `сумма подтверждённых
+ заявленный sizeBytes` (ранний отказ, экономит клиенту бесполезный PUT); (2) на confirm — `сумма
подтверждённых + реальный ContentLength` (сама подтверждаемая строка ещё не входит в сумму, т.к.
`confirmedAt IS NULL`). Оба случая бросают `ProjectStorageQuotaExceededError` (400) и, на confirm,
удаляют уже загруженный объект (тот же cleanup, что `AttachmentTooLargeError`). Сумма считается
`SUM(sizeBytes) WHERE confirmedAt IS NOT NULL` "живым" агрегатным запросом на каждый upload/confirm
— не денормализованным счётчиком на `Project`: при том же порядке величины (сотни вложений на
проект, не миллионы) живой SUM дешевле, чем поддерживать отдельное поле и рисковать его рассинхроном
с реальными строками (P2/P3 — это не история, которую нужно снапшотить, а всегда-актуальный агрегат).
`GET .../attachments/storage-usage` отдаёт `{usedBytes, quotaBytes}` для полоски прогресса на фронте.
