...
## 7. `required` enforcement — жёсткий backend (вариант B), но только на create

**Решение автора:** сервер обязан отвергать **create**, если у воркспейса есть `required: true`
поля без значения — не полагаемся на UI-звёздочку. Причины: «обязательное» должно значить
обязательное на **обоих** концах контракта (фронт и API — единый источник правды); ошибка видна
сразу при создании, а не превращается в дыру в данных, которую заметят через неделю на отчёте.

**Пересмотрено (frontend-эксплуатация, UX-сессия):** изначально та же проверка применялась и к
`PATCH`, на **результирующем смёрженном** наборе (существующие значения + патч) — эта часть
**отменена**. Причина: обнулить УЖЕ заполненное required-значение через API и так невозможно —
`null` не проходит per-type Zod-валидацию ни для одного `FieldType` (§4), а «удалить ключ» контракт
не умеет. Проверка на смёрженном наборе поэтому ничего реально не защищала — она только блокировала
**несвязанные** правки, если у лида был исторический пробел в НЕтронутом required-поле (поле стало
required уже после создания этого лида — легитимный, разрешённый сценарий, см. §10 шаг 6). На
практике: юзер открывает лид поменять один custom-field, а сохранить не может, пока не закроет
пробел в совсем другом поле, которое сейчас не при чём. См. ADR в `decisions.md`.

**Механика (актуальная):**

- **`POST /v1/projects`** — после сборки итогового `fields` проверить: для каждого
  `FieldDefinition{workspaceId, required: true}` соответствующий `key` присутствует и проходит
  type-валидацию (§4). Не хватает хотя бы одного → **400 `MISSING_REQUIRED_FIELDS`**,
  `details: { keys: string[] }` — тот же паттерн, что `PHASE_NOT_EMPTY`.
- **`PATCH /v1/projects/:id`** — **required больше не перепроверяется**. Тело по-прежнему типизируется
  per-key (§4) и мёржится с existing; если `fields` не передан — остальные поля обновляются как есть.
  Единственная защита от «пустого» required-значения — та же type-валидация, что и везде (null/
  неправильный тип → 400 `VALIDATION_ERROR`, тем же путём, что раньше).
- **Публичный intake (M5, `POST /v1/public/leads`)** унаследует **create**-путь (тот же сервисный
  метод) — required enforcement там сработает так же строго, как везде на create. Не переделка
  сейчас, просто заметка на будущее.
...
- **Required (§7):** create без значения required-поля → 400 `MISSING_REQUIRED_FIELDS` со списком `key`
  ...
  пытающийся обнулить required-значение (`null`) → 400 (type-валидация, не required-проверка) ·
  **регрессия**: поле стало `required` ПОСЛЕ создания лида (исторический пробел) → `PATCH` **другого**
  поля того же лида → 200, не спотыкается о пробел в нетронутом required-поле.
