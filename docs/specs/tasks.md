...
лиду, рутина агента. `assertOrgMember` для `assigneeId` (§3). `orgId` только из ALS; чужой
проект/таск → 404.
...
> **`complete` идемпотентен (обязательно).** `POST /complete` на уже завершённом таске (`done=true`)
> → **no-op, успех, нового `ActivityEvent` НЕ создаётся.** Проверять `done` до записи: если уже true —
> вернуть 200 без мутации и без события. Иначе двойной клик / retry / две вкладки дают два
> `task.completed` про один таск (дублирование вехи в ленте) либо неоправданный 409. `complete`
> должен быть безопасен к повтору — «завершить завершённое» безвредно, в отличие от «привязать
> привязанное» (project-links §5.2, где 409, потому что там намерение двусмысленно). `reopen`
> симметрично идемпотентен: reopen на `done=false` → no-op.
